...
async def refresh_auth_session(payload: RefreshTokenRequest, request: Request):
    record, user = validate_refresh_token(payload["refresh_token"])
    role = record["role"]
    revoke_refresh_token(record, "rotated")
    access_token = create_access_token(user["id"], role)
    ...
    record_audit_event(
        ...
        target_type=role,
        target_id=user["id"],
        metadata={"refresh_token_family": record.get("family_id")},
        request=request,
    )
    await persist_runtime_state()
    return {
...
async def logout_auth_session(payload: RefreshTokenRequest, request: Request):
    ...
    if token:
        ...
        if record:
            record_audit_event(
                action="auth.logout",
                actor={"id": record.get("user_id"), "role": record.get("role")},
                target_type=record.get("role"),
                target_id=record.get("user_id"),
                request=request,
            )
        revoke_refresh_token(record, "logout")
        await persist_runtime_state()
